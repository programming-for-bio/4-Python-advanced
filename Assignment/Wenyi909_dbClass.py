...
class Assembler5():
    """
    An object for constructing a debuijn graph from kmers of random reads, testing different parameters, and plotting a toyplot gragh
    """
    ...
    def test(self, s1, s2, s3, n1, n2, n3, k1, k2, k3):
        "Test parameters to see if that correctly generates the true target"
    
        result_dict = {}
    
        for target_size in [s1, s2, s3]:
            data = Assembler5(target_size)
        
            for nreads in [n1, n2, n3]:
                for k in [k1, k2, k3]:
                    data.run(nreads = nreads, rlen = 20, k=k)
                
                    ## store result in dict
                    result = data.assembly == data.target
                    result_dict[(target_size, nreads, k)]
