...
"""
Functions for nb 4.3 for Nicolas Locatelli.
"""
...
class Assembler5():
    ...
    def test(self, nreads, kmer):
        result_dict={}
        for target_size in [200, 500, 1000]:
            data = Assembler4(target_size)
            for nreads in [500, 1000, 5000]:
                for kmer in [10, 20, 30]:
                    data.run(nreads=nreads, rlen=50, kmer=kmer)
                    result = data.assembly == data.target
                    result_dict[(target_size, nreads, kmer)] = result 
        return result_dict        
